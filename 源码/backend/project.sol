...
contract Welfare {
  ...
  // // 添加用户
  // function addUser (address _address) public returns (bool) {
  //   uint userId = setId();
  //   userInfo[_address] = user(userId, "爱心人士", "软件工程师", "sadsa", "3");

  //   return true;
  // }

  // // 获取用户信息 
  // function getUserInfo (address _id) public view returns (uint, string, string, string, string) {
  //     user memory temp = userInfo[_id];
       
  //     return (temp.id, temp.name, temp.job, temp.avatar, temp.level);
  // } fucking the word
  ...
  function createProject (string _name, string _avatar, string _sort,
  ...
  struct hotProject {
    ...
    uint[] proofList;  // 证明列表
    mapping(uint => proof) proofa;  // 所对应的证明信息
